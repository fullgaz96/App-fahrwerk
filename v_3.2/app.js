function safeDate(dateStr) {
  if (!dateStr) return Date.now(); // falls leer -> heute
...
function addReifen() {
  const name = getVal("rl_name").trim();
  const breite = getVal("rl_breite").trim();
  const mischung = getVal("rl_mischung").trim();
  if (!name || !breite) return alert("Name und Breite sind erforderlich!");

  // ✅ Eindeutige ID mit Timestamp
  const id = "R" + Date.now();
...
function collectFormData() {
  ...
  const vrID = vrSel ? vrSel.value : "-";
  const hrID = hrSel ? hrSel.value : "-";
  ...
  return {
    // --- Grunddaten ---
    betriebsstunden: getVal("betriebsstunden", "number"),
    kilometerstand: getVal("kilometerstand", "number"),
    bestzeitMin: getVal("bestzeitMin", "number"),
    bestzeitSek: getVal("bestzeitSek", "number"),
    bestzeitZehntel: getVal("bestzeitZehntel", "number"),

    // --- Reifen ---
    reifenVorne: vr ? vr.name : "-",
    reifenVorneID: vrID || "-",
    reifenHinten: hr ? hr.name : "-",
    reifenHintenID: hrID || "-",
    luftdruckVorne: getVal("luftdruckVorne", "number"),
    luftdruckHinten: getVal("luftdruckHinten", "number"),

    // --- Gabel ---
    gabelHoehe: getVal("gabelHoehe", "number"),
    sagVStatisch: getVal("sagVStatisch", "number"),
    sagVDynamisch: getVal("sagVDynamisch", "number"),
    federV: getVal("federV"),
    vorspannungV: getVal("vorspannungV", "number"),
    druckV: getVal("druckV"),
    zugV: getVal("zugV"),
    oelViskositaet: getVal("oelViskositaet"),
    luftpolster: getVal("luftpolster", "number"),
    gabelBem: getVal("gabelBem"),

    // --- Federbein ---
    heckhoehe: getVal("heckhoehe", "number"),
    sagHStatisch: getVal("sagHStatisch", "number"),
    sagHDynamisch: getVal("sagHDynamisch", "number"),
    federH: getVal("federH"),
    vorspannungH: getVal("vorspannungH", "number"),
    druckH_ls: getVal("druckH_ls"),
    druckH_hs: getVal("druckH_hs"),
    zugH: getVal("zugH", "number"),
    federBem: getVal("federBem"),

    // --- Antrieb ---
    ritzel: getVal("ritzel", "number"),
    kettenrad: getVal("kettenrad", "number"),
    glieder: getVal("glieder", "number"),
    antriebBem: getVal("antriebBem"),

    // --- Gesamt ---
    gesamtBem: getVal("gesamtBem")
  };
}
...
function renderHistorie(){
 ...
 const sorted = setupData.historie
  ...
  .sort((a, b) => {
    const da = safeDate(a.e.datum);
    const db = safeDate(b.e.datum);
...
function getDifferences(curr, prev){
  ...
  const makeLabel = r => r 
    ? `${r.name} ${r.breite}mm ${r.mischung} (T:${r.turns}/H:${r.heizzyklen})`
    : "-";
...
function fillFormFromData(data) {
  ...
  // --- Grunddaten ---
  setVal("betriebsstunden", data.betriebsstunden);
  setVal("kilometerstand", data.kilometerstand);
  setVal("bestzeitMin", data.bestzeitMin);
  setVal("bestzeitSek", data.bestzeitSek);
  setVal("bestzeitZehntel", data.bestzeitZehntel);
  ...
  setVal("luftdruckVorne", data.luftdruckVorne);
  setVal("luftdruckHinten", data.luftdruckHinten);

  // --- Gabel ---
  setVal("gabelHoehe", data.gabelHoehe);
  setVal("sagVStatisch", data.sagVStatisch);
  setVal("sagVDynamisch", data.sagVDynamisch);
  setVal("federV", data.federV);
  setVal("vorspannungV", data.vorspannungV);
  setVal("druckV", data.druckV);
  setVal("zugV", data.zugV);
  setVal("oelViskositaet", data.oelViskositaet);
  setVal("luftpolster", data.luftpolster);
  setVal("gabelBem", data.gabelBem);

  // --- Federbein ---
  setVal("heckhoehe", data.heckhoehe);
  setVal("sagHStatisch", data.sagHStatisch);
  setVal("sagHDynamisch", data.sagHDynamisch);
  setVal("federH", data.federH);
  setVal("vorspannungH", data.vorspannungH);
  setVal("druckH_ls", data.druckH_ls);
  setVal("druckH_hs", data.druckH_hs);
  setVal("zugH", data.zugH);
  setVal("federBem", data.federBem);

  // --- Antrieb ---
  setVal("ritzel", data.ritzel);
  setVal("kettenrad", data.kettenrad);
  setVal("glieder", data.glieder);
  setVal("antriebBem", data.antriebBem);

  // --- Gesamt ---
  setVal("gesamtBem", data.gesamtBem);
}
